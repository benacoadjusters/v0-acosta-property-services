export const translations = {
  es: {
    // Navigation
    nav: {
      home: "Inicio",
      services: "Servicios",
      about: "Nosotros",
      gallery: "Galería",
      faq: "Preguntas",
      contact: "Contacto",
    },
    // Service Categories
    serviceCategories: {
      allServices: "Todos los Servicios",
      pestControl: "Control de Plagas",
      residential: "Residencial",
      commercial: "Comercial",
      commonPests: "Plagas que Controlamos",
      fumigation: "Fumigación",
      traps: "Exterminación por Trampas",
      methods: "Métodos de Control",
    },
    // Common UI
    ui: {
      getQuote: "Cotización Gratis",
      callNow: "Llamar Ahora",
      learnMore: "Saber Más",
      viewAll: "Ver Todos",
      contactUs: "Contáctanos",
      sendMessage: "Enviar Mensaje",
      phone: "Teléfono",
      email: "Correo",
      address: "Dirección",
      hours: "Horario",
      openMenu: "Abrir menú",
      closeMenu: "Cerrar menú",
    },
    // Homepage
    home: {
      heroTitle: "Cuidamos Tu Propiedad Como Merece",
      heroSubtitle: "Servicios profesionales de control de plagas, jardinería y mantenimiento para hogares y negocios en todo Puerto Rico. 4 años de experiencia protegiendo familias y empresas.",
      trustedBy: "Confiado por más de 300 clientes en Puerto Rico",
      statsYears: "Años de Experiencia",
      statsClients: "Clientes Atendidos",
      statsMunicipios: "Municipios Cubiertos",
      statsIsland: "Toda la Isla",
      statsIslandCoverage: "Cobertura Total",
      whyChooseUs: "¿Por Qué Elegirnos?",
      whyChooseUsDesc: "Nos dedicamos a brindar los mejores servicios de mantenimiento de propiedades en Puerto Rico con un enfoque en resultados y satisfacción del cliente.",
      benefit1Title: "Respuesta Rápida",
      benefit1Desc: "Servicio ágil y puntual. Atendemos tus necesidades de plagas, jardinería y mantenimiento cuando más lo necesitas.",
      benefit2Title: "Equipo Profesional",
      benefit2Desc: "Técnicos capacitados y certificados en control de plagas, jardinería y mantenimiento de propiedades.",
      benefit3Title: "Garantía de Satisfacción",
      benefit3Desc: "Nos comprometemos con resultados. Si no estás satisfecho, regresamos sin costo adicional.",
      benefit4Title: "Soluciones Integrales",
      benefit4Desc: "Un solo proveedor para todas las necesidades de tu propiedad: plagas, jardines y mantenimiento general.",
      servicesTitle: "Nuestros Servicios",
      servicesDesc: "Ofrecemos soluciones completas de mantenimiento y protección para todo tipo de propiedades.",
      coverageTitle: "Cobertura en Toda la Isla",
      coverageDesc: "Servimos a los 78 municipios de Puerto Rico. No importa dónde estés, llegamos a ti.",
      ctaTitle: "Protege Tu Propiedad Hoy",
      ctaDesc: "Solicita una cotización gratis y sin compromiso. Nuestros expertos están listos para ayudarte.",
    },
    // Services
    services: {
      title: "Nuestros Servicios",
      subtitle: "Soluciones profesionales de control de plagas para hogares y negocios",
      pestControlTitle: "Control de Plagas General",
      pestControlDesc: "Eliminación completa de todo tipo de plagas con tratamientos efectivos y seguros.",
      residentialTitle: "Servicio Residencial",
      residentialDesc: "Protección completa para tu hogar y familia contra todo tipo de plagas.",
      commercialTitle: "Servicio Comercial",
      commercialDesc: "Mantenemos tu negocio libre de plagas cumpliendo con todas las regulaciones.",
      processTitle: "Nuestro Proceso",
      step1: "Inspección",
      step1Desc: "Evaluamos tu propiedad para identificar el problema.",
      step2: "Plan de Acción",
      step2Desc: "Desarrollamos un plan personalizado para tu situación.",
      step3: "Tratamiento",
      step3Desc: "Aplicamos el tratamiento más efectivo y seguro.",
      step4: "Seguimiento",
      step4Desc: "Verificamos los resultados y garantizamos tu satisfacción.",
    },
    // About
    about: {
      title: "Sobre Nosotros",
      subtitle: "Protegiendo Puerto Rico Desde 2022",
      story: "Fundada en 2022, Acosta Property Services nació con una misión simple: brindar a Puerto Rico servicios profesionales de control de plagas en los que familias y empresas puedan confiar. En solo 4 años, hemos crecido de una pequeña operación familiar a servir a más de 300 clientes satisfechos en toda la isla.",
      missionTitle: "Nuestra Misión",
      mission: "Proteger hogares y negocios puertorriqueños con servicios de control de plagas efectivos, seguros y confiables, mientras brindamos un servicio al cliente excepcional.",
      valuesTitle: "Nuestros Valores",
      value1: "Integridad",
      value1Desc: "Honestidad y transparencia en todo lo que hacemos.",
      value2: "Excelencia",
      value2Desc: "Compromiso con la calidad en cada servicio.",
      value3: "Responsabilidad",
      value3Desc: "Cuidamos tu propiedad como si fuera nuestra.",
      teamTitle: "Nuestro Equipo",
      teamDesc: "Un equipo dedicado a brindar el mejor servicio de control de plagas en Puerto Rico.",
    },
    // Gallery
    gallery: {
      title: "Galería de Trabajo",
      subtitle: "Vea ejemplos de nuestro trabajo profesional",
      filterAll: "Todos",
      filterResidential: "Residencial",
      filterCommercial: "Comercial",
      filterTreatment: "Tratamientos",
    },
    // FAQ
    faq: {
      title: "Preguntas Frecuentes",
      subtitle: "Respuestas a las preguntas más comunes sobre nuestros servicios",
      stillHaveQuestions: "¿Tienes más preguntas?",
      contactTeam: "Nuestro equipo está listo para ayudarte.",
    },
    // Contact
    contact: {
      title: "Contáctanos",
      subtitle: "Estamos listos para ayudarte con cualquier problema de plagas",
      formTitle: "Solicita Tu Cotización Gratis",
      nameLabel: "Nombre Completo",
      namePlaceholder: "Tu nombre",
      emailLabel: "Correo Electrónico",
      emailPlaceholder: "tu@email.com",
      phoneLabel: "Teléfono",
      phonePlaceholder: "(787) 000-0000",
      serviceLabel: "Tipo de Servicio",
      servicePlaceholder: "Selecciona un servicio",
      propertyLabel: "Tipo de Propiedad",
      propertyResidential: "Residencial",
      propertyCommercial: "Comercial",
      messageLabel: "Mensaje",
      messagePlaceholder: "Describe tu situación...",
      submitButton: "Enviar Solicitud",
      successMessage: "¡Gracias! Te contactaremos pronto.",
      infoTitle: "Información de Contacto",
      hoursTitle: "Horario de Atención",
      weekdays: "Lunes - Viernes",
      saturday: "Sábados",
      sunday: "Domingos",
      closed: "Cerrado",
      emergencyTitle: "Emergencias 24/7",
      emergencyDesc: "Para emergencias de plagas fuera de horario, llámanos directamente.",
    },
    // Footer
    footer: {
      description: "Servicios profesionales de control de plagas, jardinería y mantenimiento de propiedades para hogares y negocios en todo Puerto Rico.",
      servicesTitle: "Servicios",
      companyTitle: "Empresa",
      resourcesTitle: "Recursos",
      legalTitle: "Legal",
      privacy: "Política de Privacidad",
      terms: "Términos y Condiciones",
      copyright: "Todos los derechos reservados.",
      newsletter: "Suscríbete a nuestro boletín",
      emailPlaceholder: "Tu correo electrónico",
      subscribe: "Suscribir",
    },
  },
  en: {
    // Navigation
    nav: {
      home: "Home",
      services: "Services",
      about: "About",
      gallery: "Gallery",
      faq: "FAQ",
      contact: "Contact",
    },
    // Service Categories
    serviceCategories: {
      allServices: "All Services",
      pestControl: "Pest Control",
      residential: "Residential",
      commercial: "Commercial",
      commonPests: "Pests We Control",
      fumigation: "Fumigation",
      traps: "Trap Extermination",
      methods: "Control Methods",
    },
    // Common UI
    ui: {
      getQuote: "Get Free Quote",
      callNow: "Call Now",
      learnMore: "Learn More",
      viewAll: "View All",
      contactUs: "Contact Us",
      sendMessage: "Send Message",
      phone: "Phone",
      email: "Email",
      address: "Address",
      hours: "Hours",
      openMenu: "Open menu",
      closeMenu: "Close menu",
    },
    // Homepage
    home: {
      heroTitle: "We Care for Your Property as It Deserves",
      heroSubtitle: "Professional pest control, landscaping and maintenance services for homes and businesses across Puerto Rico. 4 years of experience protecting families and companies.",
      trustedBy: "Trusted by over 300 clients in Puerto Rico",
      statsYears: "Years Experience",
      statsClients: "Clients Served",
      statsMunicipios: "Municipalities Covered",
      statsIsland: "Entire Island",
      statsIslandCoverage: "Full Coverage",
      whyChooseUs: "Why Choose Us",
      whyChooseUsDesc: "We are dedicated to providing the best property maintenance services in Puerto Rico with a focus on results and customer satisfaction.",
      benefit1Title: "Fast Response",
      benefit1Desc: "Agile and punctual service. We handle your pest, landscaping and maintenance needs when you need it most.",
      benefit2Title: "Professional Team",
      benefit2Desc: "Trained and certified technicians in pest control, landscaping and property maintenance.",
      benefit3Title: "Satisfaction Guarantee",
      benefit3Desc: "We are committed to results. If you are not satisfied, we come back at no additional cost.",
      benefit4Title: "Comprehensive Solutions",
      benefit4Desc: "One provider for all your property needs: pests, gardens and general maintenance.",
      servicesTitle: "Our Services",
      servicesDesc: "We offer complete maintenance and protection solutions for all types of properties.",
      coverageTitle: "Island-Wide Coverage",
      coverageDesc: "We serve all 78 municipalities of Puerto Rico. No matter where you are, we come to you.",
      ctaTitle: "Protect Your Property Today",
      ctaDesc: "Request a free, no-obligation quote. Our experts are ready to help you.",
    },
    // Services
    services: {
      title: "Our Services",
      subtitle: "Professional pest control solutions for homes and businesses",
      pestControlTitle: "General Pest Control",
      pestControlDesc: "Complete elimination of all types of pests with effective and safe treatments.",
      residentialTitle: "Residential Service",
      residentialDesc: "Complete protection for your home and family against all types of pests.",
      commercialTitle: "Commercial Service",
      commercialDesc: "We keep your business pest-free while complying with all regulations.",
      processTitle: "Our Process",
      step1: "Inspection",
      step1Desc: "We evaluate your property to identify the problem.",
      step2: "Action Plan",
      step2Desc: "We develop a personalized plan for your situation.",
      step3: "Treatment",
      step3Desc: "We apply the most effective and safe treatment.",
      step4: "Follow-up",
      step4Desc: "We verify results and guarantee your satisfaction.",
    },
    // About
    about: {
      title: "About Us",
      subtitle: "Protecting Puerto Rico Since 2022",
      story: "Founded in 2022, Acosta Property Services was born with a simple mission: to provide Puerto Rico with professional pest control services that families and businesses can trust. In just 4 years, we have grown from a small family operation to serving over 300 satisfied customers across the island.",
      missionTitle: "Our Mission",
      mission: "To protect Puerto Rican homes and businesses with effective, safe, and reliable pest control services while providing exceptional customer service.",
      valuesTitle: "Our Values",
      value1: "Integrity",
      value1Desc: "Honesty and transparency in everything we do.",
      value2: "Excellence",
      value2Desc: "Commitment to quality in every service.",
      value3: "Responsibility",
      value3Desc: "We care for your property as if it were our own.",
      teamTitle: "Our Team",
      teamDesc: "A dedicated team providing the best pest control service in Puerto Rico.",
    },
    // Gallery
    gallery: {
      title: "Work Gallery",
      subtitle: "See examples of our professional work",
      filterAll: "All",
      filterResidential: "Residential",
      filterCommercial: "Commercial",
      filterTreatment: "Treatments",
    },
    // FAQ
    faq: {
      title: "Frequently Asked Questions",
      subtitle: "Answers to the most common questions about our services",
      stillHaveQuestions: "Still have questions?",
      contactTeam: "Our team is ready to help you.",
    },
    // Contact
    contact: {
      title: "Contact Us",
      subtitle: "We're ready to help you with any pest problem",
      formTitle: "Request Your Free Quote",
      nameLabel: "Full Name",
      namePlaceholder: "Your name",
      emailLabel: "Email",
      emailPlaceholder: "your@email.com",
      phoneLabel: "Phone",
      phonePlaceholder: "(787) 000-0000",
      serviceLabel: "Service Type",
      servicePlaceholder: "Select a service",
      propertyLabel: "Property Type",
      propertyResidential: "Residential",
      propertyCommercial: "Commercial",
      messageLabel: "Message",
      messagePlaceholder: "Describe your situation...",
      submitButton: "Submit Request",
      successMessage: "Thank you! We'll contact you soon.",
      infoTitle: "Contact Information",
      hoursTitle: "Business Hours",
      weekdays: "Monday - Friday",
      saturday: "Saturdays",
      sunday: "Sundays",
      closed: "Closed",
      emergencyTitle: "24/7 Emergencies",
      emergencyDesc: "For pest emergencies after hours, call us directly.",
    },
    // Footer
    footer: {
      description: "Professional pest control, landscaping and property maintenance services for homes and businesses across Puerto Rico.",
      servicesTitle: "Services",
      companyTitle: "Company",
      resourcesTitle: "Resources",
      legalTitle: "Legal",
      privacy: "Privacy Policy",
      terms: "Terms and Conditions",
      copyright: "All rights reserved.",
      newsletter: "Subscribe to our newsletter",
      emailPlaceholder: "Your email",
      subscribe: "Subscribe",
    },
  },
} as const

export type Language = keyof typeof translations
export type Translations = typeof translations.es
